...
// -----------------------
// Replace with your real Anti-Captcha API key
...
/******************************************************
 * Main Puppeteer Flow with Anti-Captcha Integration
 * Accepts parameters from the frontend.
 ******************************************************/
async function fetchCaseDetailsWithAntiCaptcha({ courtIndex, caseIndex, caseNumber, caseYear }) {
  ...
  // Look up the dropdown values from our arrays using the provided indices
  ...
  console.log("Clicked submit. Waiting for result page...");
  await delay(5000);

  // Retrieve case details from the result page
  let caseDetails = "";
  try {
    const detailsEl = await page.$(".case-details");
    if (detailsEl) {
      caseDetails = await page.evaluate(el => el.innerText, detailsEl);
    }
  } catch (err) {
    console.log("Error reading case details:", err.message);
  }

  // Optionally log any error messages from the page
  try {
    const errorInfoEl = await page.$(".errorInfo");
    if (errorInfoEl) {
      const errorText = await page.evaluate(el => el.innerText, errorInfoEl);
      if (errorText) console.log("errorInfo found on page:", errorText);
    }
  } catch (err) {}

  try {
    const alertEl = await page.$(".alert");
    if (alertEl) {
      const alertText = await page.evaluate(el => el.innerText, alertEl);
      console.log("alert found on page:", alertText);
    }
  } catch (err) {}

  if (caseDetails && !caseDetails.includes("No case details found")) {
    console.log("\nCase Details:\n", caseDetails);
  } else {
...
app.post("/run-case", async (req, res) => {
  ...
  try {
    const caseDetails = await fetchCaseDetailsWithAntiCaptcha({ courtIndex, caseIndex, caseNumber, caseYear });
